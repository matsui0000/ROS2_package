...
void InverseKinematics(float x, float y){
    ...
    float l2 = param.link[1];
    float l3 = param.link[2];
    float x_temp = x - l1; //座標をq1を原点とした座標系に変換
    float A = (x_temp * x_temp + y * y + l2 * l2 - l3 * l3) / (2.0f * l2);
    float target_q0 = acosf(A / (sqrtf(x_temp * x_temp + y * y))) + atan2f(y, x_temp);  //エルボーアップ
    //float target_q0 = -acosf(A / (sqrtf(x_temp * x_temp + y * y))) + atan2f(y, x_temp); //エルボーダウン
    float target_q1 = atan2f(y - l2 * sinf(target_q0), x_temp - l2 * cosf(target_q0)) - target_q0;
...
void VisualFeedbackControl_position() {
    ...
    param.J[1][0] = l2 * cosf(q0) + l3 * cosf(q0 + q1);
    param.J[1][1] = l3 * cosf(q0 + q1);

    det_J = param.J[0][0] * param.J[1][1] - param.J[0][1] * param.J[1][0];
    if(fabs(det_J) < 0.001F){ //特異点に近い場合は逆行列を計算せずに終了
        return;
    }

    //delta_q[0]:台座側の関節角度の偏差, delta_q[1]:手先側の関節角度の偏差
    delta_Q[0] = (param.J[1][1] * delta_P[0] - param.J[0][1] * delta_P[1]) / det_J; 
    delta_Q[1] = (-param.J[1][0] * delta_P[0] + param.J[0][0] * delta_P[1]) / det_J;

    orientation.target.q[0] += delta_Q[0];
    orientation.target.q[1] += delta_Q[1];

    return; //VisualFeedbackControl_position()
}
...
void ArmControlNode::control_loop_P(){
    ...
    if(key=='b'){
        ...
        setup_flag = true;
        VisualFeedback_position_flag = false;
        VisualFeedback_angle_flag = true;
        PressureFeedback_flag = true;
        // 第1関節角(台座側)
    ...
    if(key == 'p'){
        PressureFeedbackReset();
        setup_flag = true;
        VisualFeedback_position_flag = true;
        VisualFeedback_angle_flag = true;
        PressureFeedback_flag = true;
    ...
    if(VisualFeedback_angle_flag){
        ...
        for (int i = 0; i < DEGREE_OF_FREEDOM; i++){
            double required_torque = VisualFeedbackControl_angle(orientation.target.q[i], orientation.current.q[i], i);
