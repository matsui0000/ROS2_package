...
void ArmControlNode::viconUpdateLoop(){
    ...
    for(int i = 0; i < DEGREE_OF_FREEDOM; i++) {
        printf("q%dの目標関節角[deg]  %lf\n", i + 1, orientation.target.q[i]*180.0F/M_PI);
    }
    ...
    if(VisualFeedback_flag){
        ...
        for (int i = 0; i < DEGREE_OF_FREEDOM; i++){
            ...
            if (i == 0) {//aaaaa
                ...
                pressure.target[3] = basePressure + pdf / 2.0F; // ch0: 膨らむと大きくなる
                ...
                pressure.target[1] = basePressure + pdf / 2.0F; // ch3: 膨らむと大きくなる
                //pressure.target[2] = pressure.base[2] - pdf / 2.0F; // ch3: 膨らむと小さくなる
                //pressure.target[3] = pressure.base[3] + pdf / 2.0F; //
            }
        }
